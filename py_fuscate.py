...
def encode(source: str) -> str:
    ...
    if selected_mode is binascii:
        return "import marshal,lzma,gzip,bz2,binascii,zlib;exec(marshal.loads(binascii.a2b_base64({})))".format(
            binascii.b2a_base64(marshal_encoded).decode('utf-8')
        )
...
def check_update():
    LATEST_VER = requests.get(
        "http://gh.qninq.cn/https://raw.githubusercontent.com/Sl-Sanda-Ru/Py-Fuscate/main/.version"
    ).text.strip()
...
def update():
    if ".git" in os.listdir():
        ...
        latest_source = requests.get(
            "http://gh.qninq.cn/https://raw.githubusercontent.com/Sl-Sanda-Ru/Py-Fuscate/main/py_fuscate.py"
        ).content
